...
export const CategoryCard: React.FC<CategoryCardProps> = ({ category }) => {
  ...
  return (
    <Link to={`/packages/category/${category.id}`} className="block h-full cursor-pointer">
      <motion.div
        whileHover={{ y: -8 }}
      ...
      >
        ...
        <div
          className="inline-flex items-center gap-2 text-sm font-semibold text-teal-accent group-hover:text-primary-navy group-hover:translate-x-1.5 transition-all duration-300 w-fit font-sans"
        >
          Explore Packages
